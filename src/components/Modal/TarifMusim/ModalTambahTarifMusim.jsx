...
import { createTarifMusim } from "@/api/api";
...
const ModalTambahTarifMusim = ({ onClose, musimOptions }) => {
  const { token } = useGetCookie();
  const musim_idRef = React.useRef(null);
  const jenis_kamar_idRef = React.useRef(null);
  const hargaRef = React.useRef(null);

  const handleAdd = ({ musim_id, jenis_kamar_id, harga }) => {
    createTarifMusim(token, { musim_id, jenis_kamar_id, harga })
      .then((res) => {
        console.log(res);

        if (res.status === 200 || res.status === 201) {
          toast.success("Berhasil menambahkan TarifMusim");
          onClose();
          setTimeout(() => {
  ...
  const handleSubmit = (e) => {
    ...
    handleAdd({ musim_id, jenis_kamar_id, harga });
  };
          ...
          <div className="bg-white border rounded-lg shadow relative">
            ...
            >
              ...
              <div>
                ...
                <ReactSelect options={musimOptions} ref={musim_idRef} />
              </div>
              <div>
                <label
                  for="jenis_kamar_id"
                ...
                <ReactSelect options={kamarOptions} ref={jenis_kamar_idRef} />
              </div>
              <div>
                <label
                  for="harga"
...
export default ModalTambahTarifMusim;
